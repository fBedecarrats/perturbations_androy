...
Using [the GDAL Virtual File System driver for S3](https://gdal.org/user/virtual_file_systems.html#vsis3-aws-s3-files), the access to files stores in S3 is straightforward: one just need to specify the location on the S3 bucket like if it was on the local filesystem and add "/vsis3/" at the beginning.
**Nota bene:** the credentials to access the S3 storage must be set (it is automatic on my cloud environment, but otherwise it needs to be specified manually).
